    ...
    public static void AddDbProvidersRegistration(this IServiceCollection services, PosConfiguration configuration)
    {
        switch (configuration.Database.DbProvider)
        {
            case DbProvider.PostgreSql:
                services.AddDbContext<IPosDbContext, PosDbContext>(o =>
                {
                    o.UseNpgsql(configuration.Database.BuildConnectionString() ?? string.Empty);
                });
                services.AddDbContext<IUsersDbContext, UsersDbContext>(o =>
                {
                    o.UseNpgsql(configuration.Database.BuildConnectionString() ?? string.Empty);
                });
                break;
            case DbProvider.MsSql:
                services.AddDbContext<IPosDbContext, PosDbContext>(o =>
                {
                    o.UseSqlServer(configuration.Database.BuildConnectionString() ?? string.Empty);
                });
                services.AddDbContext<IUsersDbContext, UsersDbContext>(o =>
                {
                    o.UseSqlServer(configuration.Database.BuildConnectionString() ?? string.Empty);
            ...
            case DbProvider.SqLlite:
                services.AddDbContext<IPosDbContext, PosDbContext>(o =>
                {
                    o.UseSqlite(configuration.Database.BuildConnectionString() ?? string.Empty);
                });
                services.AddDbContext<IUsersDbContext, UsersDbContext>(o =>
                {
                    o.UseSqlite(configuration.Database.BuildConnectionString() ?? string.Empty);
            ...
            default:
                services.AddDbContext<IPosDbContext, PosDbContext>(o =>
                {
                    o.UseInMemoryDatabase(configuration.Database.BuildConnectionString() ?? string.Empty);
                });
                services.AddDbContext<IUsersDbContext, UsersDbContext>(o =>
                {
                    o.UseInMemoryDatabase(configuration.Database.BuildConnectionString() ?? string.Empty);
